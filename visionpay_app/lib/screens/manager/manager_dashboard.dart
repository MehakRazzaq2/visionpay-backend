...
class _ManagerDashboardState extends State<ManagerDashboard> {
 ...
 final String _apiBase = 'http://192.168.0.111:8000';
  ...
  final List<Map<String, dynamic>> _staff = [
    {'name': 'Store Manager', 'role': 'Manager', 'status': 'Online', 'time': '09:00 AM'},
    {'name': 'Cashier', 'role': 'Cashier', 'status': 'Online', 'time': '09:15 AM'},
  ];
      ...
      title: Row(
        children: [
          ...
          Column(
            ...
            children: [
              Text('VisionPay', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
              Text('Manager Dashboard', style: TextStyle(fontSize: 10, color: AppColors.textSecondary)),
            ],
          ),
        ],
      ),
  ...
  Widget _buildBody() {
    switch (_selectedIndex) {
      ...
      case 2: return _buildTransactions();
      case 3: return _buildStaff();
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(widget.fullName, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
                    ],
                  ),
                ),
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 4),
                      Text('Online', style: TextStyle(fontSize: 10, color: AppColors.success, fontWeight: FontWeight.w600)),
                    ],
                  ),
                ),
              ],
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text('${_lowStock.length} product(s) need restocking',
                          style: TextStyle(fontSize: 12, color: AppColors.error, fontWeight: FontWeight.w600)),
                    ),
                    GestureDetector(
                      onTap: () => setState(() => _selectedIndex = 1),
                      child: Text('View', style: TextStyle(fontSize: 12, color: AppColors.primary, fontWeight: FontWeight.bold)),
                    ),
                  ],
                ),
              ),
            ...
            GridView.count(
              ...
              children: [
                _clickableStatCard(
                  ...
                  value: 'Rs ${(_stats['today_revenue'] ?? 0).toStringAsFixed(0)}',
                ...
                _clickableStatCard(
                  ...
                  color: Color.fromARGB(255, 220, 158, 114),
                  onTap: _showSalesDetails,
                ),
                _clickableStatCard(
                  title: 'Monthly Revenue',
                  value: 'Rs ${(_stats['monthly_revenue'] ?? 0).toStringAsFixed(0)}',
                ...
                _clickableStatCard(
                  ...
                  color: Color.fromARGB(255, 210, 136, 97),
            ...
            Row(
              ...
              children: [
                ...
                GestureDetector(
                  ...
                  child: Text('See All', style: TextStyle(fontSize: 12, color: AppColors.primary, fontWeight: FontWeight.w600)),
                ),
              ],
            ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(title, style: TextStyle(fontSize: 9, color: Colors.white.withOpacity(0.85))),
                ],
              ),
            ),
            Icon(Icons.chevron_right, color: Colors.white.withOpacity(0.7), size: 14),
          ],
        ),
      ),
    );
  ...
  Widget _buildProducts() {
    final weightBased = _products.where((p) => p['weight_based'] == true).toList();
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text('Products', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
                ElevatedButton.icon(
            ...
            Row(
              children: [
                Expanded(child: _miniCard('Total', '${_products.length}', Icons.category_outlined, AppColors.primary)),
                const SizedBox(width: 8),
                Expanded(child: _miniCard('Packed', '${ packed.length}', Icons.label_outline, const Color(0xFF0D9488))),
                const SizedBox(width: 8),
                Expanded(child: _miniCard('Weight', '${weightBased.length}', Icons.scale_outlined, const Color(0xFFF97316))),
                const SizedBox(width: 8),
                Expanded(child: _miniCard('Alerts', '${_lowStock.length}', Icons.warning_outlined, AppColors.error)),
              ],
            ),
            const SizedBox(height: 16),
            ...
            // Needs restocking
            if (_lowStock.isNotEmpty) ...[
              _sectionHeader('⚠️ Needs Restocking (${_lowStock.length})', AppColors.error),
            ...
            // Packed
            _sectionHeader('📦 Packed Products (${packed.length})', AppColors.primary),
            const SizedBox(height: 8),
            ...packed.map((p) => _productCard(p)),
            const SizedBox(height: 14),

            // Weight based
            _sectionHeader('⚖️ Weight Based (${weightBased.length})', const Color(0xFFF97316)),
            const SizedBox(height: 8),
  ...
  Widget _productCard(dynamic p, {bool isAlert = false}) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: isAlert ? AppColors.error.withOpacity(0.3) : AppColors.border.withOpacity(0.3)),
      ...
      child: Row(
        children: [
          Container(
            width: 38, height: 38,
            decoration: BoxDecoration(
              color: isWeight ? const Color(0xFFF97316).withOpacity(0.1) : AppColors.primary.withOpacity(0.1),
              borderRadius: BorderRadius.circular(10),
            ),
            child: Icon(
              isWeight ? Icons.scale_outlined : Icons.label_outline,
              color: isWeight ? const Color(0xFFF97316) : AppColors.primary,
              size: 18,
            ),
          ),
          const SizedBox(width: 10),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(name ?? '', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.textPrimary, fontSize: 13)),
                Text(
                  isWeight ? price : '$price • Stock: $qty',
                  style: TextStyle(fontSize: 11, color: isAlert ? AppColors.error : AppColors.textSecondary),
                ),
                if (remark.isNotEmpty)
                  Text(remark, style: TextStyle(fontSize: 10, color: isAlert ? AppColors.error : AppColors.success)),
              ],
            ),
          ),
          Row(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Tap any transaction for details',
  ...
  List<Map<String, dynamic>> _getAllTransactions() {
    ...
    // Merge fake + real (fake first for demo)
    return [..._fakeTransactions, ...realTx];
  }
      ...
      children: [
        ...
        Expanded(
          child: Column(
            ...
            children: [
              Text(billId,
                  style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.textPrimary, fontSize: 12)),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text('${tx['items_count']} items • Cashier: ${tx['cashier'] ?? 'N/A'}',
            ...
            Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text('Paid', style: TextStyle(fontSize: 10, color: AppColors.success, fontWeight: FontWeight.w600)),
                ),
              ],
            ),
          ],
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _detailRow('Items', '${tx['items_count']} products', AppColors.textPrimary),
            _detailRow('Payment', tx['payment'] ?? 'Cash', AppColors.success),
            _detailRow('Status', tx['paid'] == true ? '✅ Paid' : '❌ Unpaid', AppColors.success),
            _detailRow('Time', tx['timestamp'] ?? 'N/A', AppColors.textSecondary),
            if (tx['items'] != null && tx['items'].toString().isNotEmpty) ...[
              const Divider(height: 20),
              Text('Items:', style: TextStyle(fontWeight: FontWeight.bold, color: AppColors.textPrimary, fontSize: 13)),
              const SizedBox(height: 6),
              Text(tx['items'].toString(), style: TextStyle(fontSize: 12, color: AppColors.textSecondary)),
            ],
            const Divider(height: 20),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
  ...
  Widget _buildStaff() {
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Staff Overview', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
          const SizedBox(height: 4),
          Text('Active staff members today', style: TextStyle(fontSize: 13, color: AppColors.textSecondary)),
          const SizedBox(height: 16),
          Row(
            children: [
              Expanded(child: _miniCard('Total Staff', '${_staff.length}', Icons.people_outline, AppColors.primary)),
              const SizedBox(width: 12),
              Expanded(
                  child: _miniCard('Online Now',
                      '${_staff.where((s) => s['status'] == 'Online').length}',
                      Icons.circle, AppColors.success)),
            ],
          ),
          const SizedBox(height: 20),
          Text('Staff Members', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
          const SizedBox(height: 12),
          ..._staff.map((s) => _staffCard(s)),
          const SizedBox(height: 20),
          Text("Today's Activity", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
          const SizedBox(height: 12),
          Container(
            padding: const EdgeInsets.all(16),
            decoration: BoxDecoration(
            ...
            child: Column(
              children: [
                _activityRow(Icons.login, 'Cashier logged in', '09:15 AM', AppColors.success),
                const Divider(height: 20),
                _activityRow(Icons.receipt_long, 'Transactions today',
                    '${_stats['today_transactions'] ?? 0} sales', AppColors.primary),
                const Divider(height: 20),
                _activityRow(Icons.monetization_on, 'Revenue today',
                    'Rs ${(_stats['today_revenue'] ?? 0).toStringAsFixed(0)}', AppColors.gold),
              ],
            ),
          ),
        ],
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(s['name'], style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.textPrimary, fontSize: 14)),
                Text(s['role'], style: TextStyle(fontSize: 12, color: AppColors.textSecondary)),
              ],
            ),
          ),
          Column(
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(20),
                  border: Border.all(color: isOnline ? AppColors.success.withOpacity(0.3) : Colors.grey.withOpacity(0.3)),
                ),
                child: Row(
                  children: [
                    Container(
              ...
              const SizedBox(height: 4),
              Text('Since ${s['time']}', style: TextStyle(fontSize: 10, color: AppColors.textSecondary)),
            ],
          ),
        ],
      ),
      ...
      builder: (context, setS) => SingleChildScrollView(
        ...
        child: Column(
          children: [
              ...
              child: Column(
                children: [
                  ...
                  Container(
                    ...
                    child: const Text('Store Manager', style: TextStyle(color: Colors.white, fontSize: 12)),
                  ),
                ],
              ),
            ),
            ...
            Row(
              children: [
                Expanded(child: _miniCard("Today's Sales", '${_stats['today_transactions'] ?? 0}',
                    Icons.receipt_outlined, AppColors.primary)),
                const SizedBox(width: 12),
                Expanded(child: _miniCard('Products', '${_products.length}',
                    Icons.inventory_outlined, AppColors.accent)),
                const SizedBox(width: 12),
                Expanded(child: _miniCard('Alerts', '${_lowStock.length}',
            ...
            _profileOption(Icons.store_outlined, 'Store Information',
                'VisionPay Grocery Store — KICSIT 2026', AppColors.primary, onTap: null),

            // About App — expandable
            GestureDetector(
              onTap: () => setS(() => _aboutExpanded = !_aboutExpanded),
              child: Container(
                margin: const EdgeInsets.only(bottom: 10),
                padding: const EdgeInsets.all(14),
                decoration: BoxDecoration(
                    ...
                    Row(
                      children: [
                        Container(
                          ...
                          child: Icon(Icons.info_outline, color: AppColors.textSecondary, size: 18),
                        ),
                        const SizedBox(width: 12),
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text('Tap to expand', style: TextStyle(fontSize: 11, color: AppColors.textSecondary)),
                            ],
                          ),
                        ),
                        Icon(_aboutExpanded ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down,
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(title, style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.textPrimary, fontSize: 13)),
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _sheetHeader("Today's Revenue", Icons.monetization_on_outlined, const Color(0xFF0F766E)),
            const SizedBox(height: 16),
            _detailRow('Total Revenue Today',
                'Rs ${(_stats['today_revenue'] ?? 0).toStringAsFixed(0)}', const Color(0xFF0F766E)),
            _detailRow('Total Transactions', '${_stats['today_transactions'] ?? 0}', AppColors.accent),
            _detailRow('Average per Sale',
                (_stats['today_transactions'] != null && (_stats['today_transactions'] as num) > 0)
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _sheetHeader("Today's Sales", Icons.receipt_outlined, const Color(0xFFF97316)),
            const SizedBox(height: 12),
            _detailRow('Total Sales Today', '${_stats['today_transactions'] ?? 0} txn', const Color(0xFFF97316)),
            const SizedBox(height: 12),
            Text('Recent Sales:', style: TextStyle(fontWeight: FontWeight.bold, color: AppColors.textPrimary, fontSize: 13)),
            const SizedBox(height: 8),
            ...allTx.map((tx) => Padding(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Expanded(child: Text(tx['bill_id'] ?? '', style: TextStyle(fontSize: 12, color: AppColors.textPrimary))),
                  Text('Rs ${(tx['total'] ?? 0).toStringAsFixed(0)}',
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _sheetHeader('Monthly Revenue', Icons.calendar_month_outlined, const Color(0xFF0D9488)),
            const SizedBox(height: 16),
            _detailRow('This Month Total',
                'Rs ${(_stats['monthly_revenue'] ?? 0).toStringAsFixed(0)}', const Color(0xFF0D9488)),
            _detailRow("Today's Revenue",
                'Rs ${(_stats['today_revenue'] ?? 0).toStringAsFixed(0)}', AppColors.primary),
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _sheetHeader('Low Stock Alert', Icons.warning_amber_outlined, const Color(0xFFEA580C)),
            const SizedBox(height: 12),
            if (_lowStock.isEmpty)
              Center(child: Text('All products well stocked! ✅',
        ...
        builder: (context, setS) => AlertDialog(
          ...
          title: Row(
            children: [
              ...
              const SizedBox(width: 8),
              Text('Add Product', style: TextStyle(color: AppColors.textPrimary, fontSize: 16)),
            ],
          ),
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                _dialogField(priceCtrl, isWeight ? 'Price per kg' : 'Price per piece', Icons.attach_money),
                const SizedBox(height: 12),
                if (!isWeight)
                  _dialogField(qtyCtrl, 'Quantity', Icons.inventory_2_outlined, isNumber: true),
                const SizedBox(height: 8),
                Row(
                  children: [
                    Switch(value: isWeight, activeColor: AppColors.primary, onChanged: (v) => setS(() => isWeight = v)),
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          children: [
            Icon(Icons.add_circle_outline, color: AppColors.primary),
            const SizedBox(width: 8),
            Text('Update Stock', style: TextStyle(color: AppColors.textPrimary, fontSize: 16)),
          ],
        ),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Product: $name', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
            const SizedBox(height: 12),
            _dialogField(ctrl, 'New Quantity', Icons.inventory_2_outlined, isNumber: true),
          ],
        ),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.pop(context);
              _updateStock(id, int.tryParse(ctrl.text) ?? 0);
            },
            style: ElevatedButton.styleFrom(
              backgroundColor: AppColors.primary,
              foregroundColor: Colors.white,
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 4),
          Text(value, style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
          Text(title, style: TextStyle(fontSize: 9, color: AppColors.textSecondary), textAlign: TextAlign.center),
        ],
      ),
    );
  }
